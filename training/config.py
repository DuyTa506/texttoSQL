...
Qwen3.5-2B with built-in thinking mode (<think>...</think>).
...
class LoRAConfig:
    ...
    r: int = 32                            # rank: 8 (fast) → 128 (complex tasks)
...
class SFTConfig:
    ...
    # Model — Qwen3.5-2B with built-in thinking
...
class RLConfig:
    ...
    num_epochs: int = 1
    max_steps: int = -1                     # -1 = use num_epochs; set >0 to override
    ...
    reward_weights: dict = field(default_factory=lambda: {
        ...
        "execution_accuracy": 5.0,         # SQL execution correct (real DB)
        "schema_faithfulness": 2.0,        # uses correct tables/columns
        "error_correction": 6.0,           # bonus for error-correction task type
    })

    # Data
    train_data_path: str = "data/rl/grpo_train.jsonl"
    db_dir: str = "datasets/data"
